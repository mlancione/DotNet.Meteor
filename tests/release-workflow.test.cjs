...
function runRelease({ tagCommit = '', exists = false, version = '6.2.11', failUpload = false } = {}) {
    ...
    try {
        ...
        fs.writeFileSync(path.join(root, 'gh'), '#!/bin/sh\nprintf "%s\\n" "$*" >> "$TEST_LOG"\nif [ "$2" = "view" ]; then [ "$TEST_EXISTS" = "yes" ]; elif [ "$2" = "upload" ] && [ "$TEST_UPLOAD_FAIL" = "yes" ]; then exit 1; else exit 0; fi\n', { mode: 0o755 });
        ...
        const result = spawnSync('bash', [script], { cwd: root, encoding: 'utf8', env: {
            ...
            GITHUB_SHA: 'built-commit', TEST_TAG_COMMIT: tagCommit, TEST_EXISTS: exists ? 'yes' : 'no', TEST_LOG: log, TEST_UPLOAD_FAIL: failUpload ? 'yes' : 'no'
        }});
