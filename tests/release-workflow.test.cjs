...
function runRelease({ tagCommit = '', exists = false, version = '6.2.11' } = {}) {
    ...
    try {
        ...
        fs.writeFileSync(path.join(root, 'gh'), '#!/bin/sh\nprintf "%s\\n" "$*" >> "$TEST_LOG"\nif [ "$2" = "view" ]; then [ "$TEST_EXISTS" = "yes" ]; else exit 0; fi\n', { mode: 0o755 });
        ...
        const result = spawnSync('bash', [script], { cwd: root, encoding: 'utf8', env: {
            ...
            GITHUB_SHA: 'built-commit', TEST_TAG_COMMIT: tagCommit, TEST_EXISTS: exists ? 'yes' : 'no', TEST_LOG: log
        }});
...
test('release targets the built commit and includes VSIX assets', () => {
    ...
    assert.match(result.calls, /release create local-v6\.2\.11 artifacts\/test\.vsix --target built-commit/);
});
